        ...
        {questions.data ? (
          !nextFormPage ? (
            <div className="form-wrapper">
              <progress className="progress is-primary" value="80" max="100">
                80%
              </progress>
              {questions.data
                ? questions.data.qset2.questions.slice(10, 15).map((q, i) => (
                    <div key={i} className="field">
                      <label className="label">
                        {q.includes("COUNTRYNAME")
                          ? q.replace("COUNTRYNAME", userFormData.country)
                          : q.includes("{I/we}") &&
                            userFormData.kindOfTrip === "Solo"
                          ? q.replace("{I/we}", "I")
                          : q.includes("{I/we}") &&
                            userFormData.kindOfTrip !== "Solo"
                          ? q.replace("{I/we}", "we")
                      ...
                      <div className="control">
                        <div className="select">
                          <select
                            ...
                            name={questions.data.qset2.name.slice(10, 15)[i]}
                            required
                          >
